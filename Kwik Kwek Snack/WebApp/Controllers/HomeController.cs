...
namespace WebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            if (drink.DrinkName == null)
            {
                return RedirectToAction("Drinks", "Home", drink);
            }
            else
            {
        ...
        {
            bool takeAway = order.TakeAway;
            order = _repo.GetOrder(order.OrderID);
            order.TakeAway = takeAway;
            order.Status = Status.Wachtrij;
